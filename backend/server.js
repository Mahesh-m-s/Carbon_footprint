    ...
    if (type === 'admin') {
      ...
      if (rows.length > 0) {
        ...
        if (rows[0].password === password) {
          ...
          console.log('❌ Incorrect admin password:', username);
    ...
    // ─── USER LOGIN ───────────────────────────────────────────────────────
      ...
      if (!username || !dob) {
        return res.status(400).json({
          success: false,
          error: 'USN and Date of Birth required'
        });
      }

      const [rows] = await pool.query(
        `SELECT *,
                DATE_FORMAT(dob, '%Y-%m-%d') AS formatted_dob
         FROM users
      ...
      if (rows.length === 0) {
        ...
        await pool.query(
          'INSERT INTO users (username, dob, role) VALUES (?, ?, ?)',
          [username, dob, 'user']
        );
      ...
      // ─── Existing User ─────────────────────────────────────────────────
        ...
        if (storedDob && storedDob !== dob) {
          ...
          console.log('❌ Incorrect DOB:', username);
        ...
        // User exists but DOB is missing
        else if (!storedDob) {

          await pool.query(
            'UPDATE users SET dob = ? WHERE username = ?',
            [dob, username]
          );

          console.log('✅ DOB updated for user:', username);
        }
      }
...
app.post('/api/devices', async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO devices
       (device_name, device_type, department_id, power_rating_watts, quantity)
       VALUES (?, ?, ?, ?, ?)`,
      [
        device_name,
        device_type,
...
app.get('/api/device-usage', async (req, res) => {
  ...
  let query = `
    ...
    JOIN devices dev
      ON du.device_id = dev.id
    LEFT JOIN departments dep
      ON dev.department_id = dep.id
    JOIN emission_factors ef
      ON ef.source_type = 'electricity'
    WHERE 1=1
  `;
...
app.post('/api/device-usage', async (req, res) => {
  ...
  if (!device_id || !usage_date || !hours_used) {
    return res.status(400).json({
      success: false,
      error: 'device_id, usage_date, hours_used required'
    });
  }
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO device_usage
       (device_id, usage_date, hours_used, recorded_by, notes)
       VALUES (?, ?, ?, ?, ?)`,
      [
        device_id,
        usage_date,
...
app.delete('/api/device-usage/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'DELETE FROM device_usage WHERE id = ?',
      [req.params.id]
    );
...
app.get('/api/internet', async (req, res) => {
  ...
  let query = `
    SELECT
      ...
      ROUND(
        ic.data_used_gb * ef.factor_value,
        4
      ) AS emission_kg
    FROM internet_consumption ic
    JOIN departments d
      ON ic.department_id = d.id
    JOIN emission_factors ef
      ON ef.source_type = 'internet_data'
    WHERE 1=1
  `;
...
app.post('/api/internet', async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO internet_consumption
       (department_id, consumption_date, data_used_gb,
        num_users, connection_type, recorded_by)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        department_id,
        consumption_date,
...
app.delete('/api/internet/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'DELETE FROM internet_consumption WHERE id = ?',
      [req.params.id]
    );
...
app.get('/api/electricity', async (req, res) => {
  ...
  let query = `
    SELECT
      ...
      d.name AS department,
      ROUND(
        eu.units_consumed_kwh * ef.factor_value,
        4
      ) AS emission_kg
    FROM electricity_usage eu
    JOIN departments d
      ON eu.department_id = d.id
    JOIN emission_factors ef
      ON ef.source_type = 'electricity'
    WHERE 1=1
  `;
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO electricity_usage
       (department_id, usage_date, units_consumed_kwh,
        meter_reading_start, meter_reading_end, recorded_by, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        department_id,
        usage_date,
...
app.delete('/api/electricity/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'DELETE FROM electricity_usage WHERE id = ?',
      [req.params.id]
    );
...
app.get('/api/dashboard/summary', async (req, res) => {
  ...
  try {
    ...
    const [elec] = await pool.query(`
      SELECT
        ROUND(
          SUM(eu.units_consumed_kwh * ef.factor_value),
          2
        ) AS total_emission_kg,

        ROUND(
          SUM(eu.units_consumed_kwh),
          2
        ) AS total_kwh
    ...
    const [inet] = await pool.query(`
      SELECT
        ROUND(
          SUM(ic.data_used_gb * ef.factor_value),
          2
        ) AS total_emission_kg,
...
app.get('/api/dashboard/monthly', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Monthly dashboard error:', err.message);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(
    `🌿 Carbon Footprint Server running on port ${PORT}`
  );
});